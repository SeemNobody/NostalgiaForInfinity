...
if [ -f "$PHASE1_PARAMS" ]; then
  ...
  EXTRA_CONFIG="--config $PHASE1_PARAMS"
else
  echo "⚠ 警告: 未找到阶段1参数文件"
  echo "  请先执行: bash scripts/run_hyperopt_phase1.sh"
  echo "  然后导出最佳参数: freqtrade hyperopt-show --best --print-json > $PHASE1_PARAMS"
  EXTRA_CONFIG=""
fi
...
echo ""
echo "导出最佳参数到JSON:"
echo "  freqtrade hyperopt-show --best --print-json > $RESULT_DIR/phase2_best_params.json"
echo ""
echo "下一步: 执行阶段3优化"
echo "  bash scripts/run_hyperopt_phase3.sh"
echo "=========================================="
